...
usage() {
  cat <<'EOF'
Install ai_resilience_evaluator for Codex and/or Claude Code.
...
Default behavior:
  If no target flag is given, install both Codex and Claude.
EOF
}
...
skill_name="ai_resilience_evaluator"
...
if [[ "$install_codex" -eq 1 ]]; then
  install_target "Codex" "$repo_root" "$codex_home/skills/$skill_name"
fi
